...
**Root Cause:** `md_parser.c` normalizes away trailing spaces before a hard line break (CommonMark spec §6.7). So `Heading  \n` → `<p>Heading<br />` without the trailing space. This is correct Markdown behaviour, but it causes a byte-exact round-trip mismatch.

**Fix needed in `md_parser.c`:** When emitting text before a `<br />`, preserve the trailing space in the HTML output. OR: when `html_serializer.c` detects that the text node is followed by `<br>`, also strip the trailing space from the text before the two-space break (the trailing-space stripper in `ser_walk_node` is already there but only covers the text-node-to-br sibling relationship, and the `  \n` still gets the trailing space normalized away by md_parser).

**Simplest fix:** In `html_serializer.c`'s `<br>` handler (line ~1070), when `<br>` is the first child of a `<p>` (i.e., `out->len == 0` or previous char is `\n`), emit `\\\n`. Otherwise emit `  \n`. The real issue is that md_parser strips the space that precedes `  \n` — this comes from md_parser.c's inline parser when handling `\n`. Search for where it handles trailing spaces on inline text before a newline.
...
**Actual root cause:** When `html_to_md` serializes `<ine >` (tag with trailing space but no attrs), the raw_html_fallback emits `<ine>` without the trailing space. The original HTML had `<ine >` but we emit `<ine>`.

**Fix:** In the raw_html_fallback serialization code (`html_serializer.c` ~line 1156), if a tag has no attributes, still emit the raw tag string as-is from `raw_tag`. But `raw_tag` only stores the tag name, not the full original tag content including trailing spaces.

Actually the right fix is simpler: the HTML parser should be treating `<ine >` exactly like `<ine>` since a trailing space inside a tag is just whitespace. And the re-serialized `<ine>` is valid equivalent HTML. The issue is that `md_parser.c` in Pass 1 sees `<ine >` as a valid HTML inline tag (with trailing space before `>`), but Pass 3 sees `<ine>` and may treat it differently.
