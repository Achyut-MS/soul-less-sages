...
static size_t find_marker_index(const char *text, const char *marker) {
    if (!text || !marker) return SIZE_MAX;
    size_t mlen = strlen(marker);
    if (mlen == 0) return SIZE_MAX;
...
static char *render_inline_fragment(const char *text, const char *src, size_t base_offset, parser_state_t *st) {
    ...
    for (size_t i = 0; i < len; ) {
        if (text[i] == '\\') {
            ...
            if (text[i + 1] == ' ' && (out[0] == '\0' || out[strlen(out) - 1] == '\n')) {
                i += 2;
                continue;
            }
            char buf[2];
        ...
        if (strncmp(text + i, "***", 3) == 0) {
            if (i + 3 >= len || text[i + 3] == ' ' || text[i + 3] == '\t' || text[i + 3] == '\n' || text[i + 3] == '\r') {
                append_str(&out, "***");
                i += 3;
                continue;
            }
            size_t close = find_marker_index(text + i + 3, "***");
            if (close == SIZE_MAX || close == 0 || text[i + 3 + close - 1] == ' ' || text[i + 3 + close - 1] == '\t' || text[i + 3 + close - 1] == '\n') {
                append_str(&out, "***");
                i += 3;
                continue;
            }
        ...
        if (strncmp(text + i, "**", 2) == 0) {
            if (i + 2 >= len || text[i + 2] == ' ' || text[i + 2] == '\t' || text[i + 2] == '\n' || text[i + 2] == '\r') {
                append_str(&out, "**");
                i += 2;
                continue;
            }
            size_t close = find_marker_index(text + i + 2, "**");
            if (close == SIZE_MAX || close == 0 || text[i + 2 + close - 1] == ' ' || text[i + 2 + close - 1] == '\t' || text[i + 2 + close - 1] == '\n') {
                append_str(&out, "**");
                i += 2;
                continue;
            }
        ...
        if (text[i] == '*') {
            ...
            if (close == SIZE_MAX || close == 0 || text[i + 1 + close - 1] == ' ' || text[i + 1 + close - 1] == '\t' || text[i + 1 + close - 1] == '\n') {
                append_char(&out, '*');
                i += 1;
                continue;
            }
        ...
        if (text[i] == '!' && i + 1 < len && text[i + 1] == '[') {
            ...
            if (close_bracket != SIZE_MAX && close_bracket + 1 < len && text[close_bracket + 1] == '(') {
                ...
                if (close_paren != SIZE_MAX) {
                    size_t alt_len = close_bracket - (i + 2);
                    size_t url_len = close_paren - (close_bracket + 2);
                    char *alt = xstrdup_len(text + i + 2, alt_len);
                    char *url = xstrdup_len(text + close_bracket + 2, url_len);
                    char *escaped_alt = xstrdup("");
                    escape_html_append(&escaped_alt, alt);
                    char *escaped_url = xstrdup("");
                    escape_html_append(&escaped_url, url);
                    free(alt);
                    free(url);
                    append_str(&out, "<img src=\"");
                    append_str(&out, escaped_url);
                    append_str(&out, "\" alt=\"");
                    append_str(&out, escaped_alt);
                    append_str(&out, "\" />");
                    free(escaped_alt);
                    free(escaped_url);
                    i = close_paren + 1;
